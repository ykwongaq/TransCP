# import debugpy;debugpy.connect(('10.119.17.124', 5628))
import argparse
import datetime
import json
import random
...
def get_args_parser():
    parser = argparse.ArgumentParser(
        "TransCP: context disentangling and prototype inheriting for robust visual grounding",
        add_help=False,
    )
    ...
    parser.add_argument(
        "--backbone_path",
        default="pretrained_checkpoints/resnet50-19c8e357.pth",
        type=str,
        help="Name of the convolutional backbone to use",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--bert_model",
        default="pretrained_checkpoints/bert-base-uncased.tar.gz",
        type=str,
        help="Bert model",
    )
    parser.add_argument(
        "--bert_token_mode",
        default="pretrained_checkpoints/bert_base_uncased",
        type=str,
        help="Bert tokenizer mode",
    )
    parser.add_argument(
        "--bert_output_dim", default=768, type=int, help="Size of the output of Bert"
    )
    parser.add_argument(
        "--bert_output_layers", default=12, type=int, help="the output layers of Bert"
    )
    parser.add_argument(
        "--max_query_len",
        default=20,
    ...
    parser.add_argument(
        "--config",
        default="configs/TransCP_R50_referit.py",
        type=str,
...
def main(args):
    ...
    epoch_logger = get_logger(
        "train_epoch", args.output_dir, utils.get_rank(), filename="epoch.log"
    )
    logger.info(args)
    ...
    vis_enc_param = [
        ...
        if p.requires_grad
        and (n.startswith("trans_encoder") or n.startswith("input_proj"))
    ]
    ...
    rest_param = [
        ...
        if p.requires_grad
        and (
            n.startswith("bbox_regression")
            or n.startswith("bbox_embed")
            or n.startswith("bert_proj")
        )
    ]
    ...
    logger.info(
        f"The num of learnable parameters: backbone({cnt_backbone}), vis_enc({cnt_vis_enc}), "
        f"bert({cnt_bert}), rest({cnt_rest})"
    )
    logger.info(
        f"Check the whole parameters: {cnt_whole} = {cnt_backbone + cnt_vis_enc + cnt_bert + cnt_rest}"
    )

    param_dicts = [
        {"params": rest_param},  # base_lr
    ...
    logger.info(
        f"The size of dataset: train({len(dataset_train)})  val({len(dataset_val)})"
    )
    ...
    if args.eval:
        print(epoch_eval)
        test_stats, test_time = epoch_eval(
            model,
            criterion,
            postprocessor,
            data_loader_val,
    ...
    if args.start_epoch < args.freeze_epochs and args.freeze_modules:
        logger.info(
            f"Freeze weights: {args.freeze_modules} and {args.freeze_param_names}"
        )
        param_freezer = ParamFreezer(args.freeze_modules, args.freeze_param_names)
        param_freezer.freeze(model_without_ddp)
        if (
            args.distributed
        ):  # re-wrap the model to avoid error: 'parameters that were not used in producing loss'
            model = torch.nn.parallel.DistributedDataParallel(
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) == args.freeze_epochs and args.freeze_modules:
            ...
            if (
                args.distributed
            ):  # re-wrap the model to ensure the same gradients for unfrozen weights
                model = torch.nn.parallel.DistributedDataParallel(
                    model_without_ddp,
                    device_ids=[args.gpu],
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        "TransCP training script", parents=[get_args_parser()]
    )
