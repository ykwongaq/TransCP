...
def main(args):
    ...
    logger.info(
        "  ".join(["[Test accuracy]", *[f"{k}: {v:.4f}" for k, v in test_acc.items()]])
    )
    logger.info(
        "  ".join(["[Test time]", *[f"{k}: {v:.6f}" for k, v in test_time.items()]])
    )
    return
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser("TransCP test script", parents=[get_args_parser()])
