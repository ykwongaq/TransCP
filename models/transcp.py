...
class TransCP(nn.Module):
    def __init__(self, pretrained_weights, args=None):
        ...
        self.input_proj = nn.Conv2d(
            self.backbone.num_channels, self.trans_encoder.d_model, kernel_size=1
        )
    ...
    def forward(self, image, image_mask, word_id, word_mask):
        ...
        img_feat, mask, pos_embed = self.trans_encoder(
            self.input_proj(src), mask, pos[-1]
        )
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_dict):
        ...
        if orig_sizes is not None:
            ...
            boxes = torch.min(
                boxes, torch.stack([orig_w, orig_h, orig_w, orig_h], dim=1)
            )
...
def build_vgmodel(args):
    ...
    criterion = VGCriterion(
        weight_dict=weight_dict, loss_loc=args.loss_loc, box_xyxy=args.box_xyxy
    )
    criterion.to(device)
